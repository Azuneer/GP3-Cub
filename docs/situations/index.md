# Situations & Activités

Productions concrètes de notre groupe pour chaque situation professionnelle du référentiel BTS SIO, organisées par bloc.

<div class="grid cards" markdown>

- :material-monitor:{ .lg .middle } __Bloc 2 - Administration Système__

    ---

    [Situation 1 : Préparation de la maquette et premiers paramétrages du serveur Windows 2025](bloc2-admin-sys/situation1-windows.md)

- :material-cog:{ .lg .middle } __Bloc 2 - Exploitation des services__

    ---

    [Activité 0 : Mise en place du contexte CUB](bloc2-services/activite0.md)

- :material-lan:{ .lg .middle } __Bloc 2 - Administration et supervision des réseaux__

    ---

    [Feuille de route des chapitres](bloc2-reseaux/feuille-de-route.md)  
    [Activité 0 : Mise en place de l'infrastructure réseau des agences](bloc2-reseaux/activite0.md)

- :material-shield-lock:{ .lg .middle } __Bloc 3 - Cybersécurité__

    ---

    [Situation 1 : Phase d'analyse préalable](bloc3-cyber/situation1.md)

</div>
